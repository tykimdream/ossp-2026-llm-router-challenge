...
# Aggressive Router V6.1 Competitive 평가와 적대적 리뷰

## 결론

V6.1 Competitive는 V5의 학습 계수, tier별 안전계수와 배정 의미를 동결한 채
운영 취약점을 보완한 버전이다.
14개 엣지케이스를 V5/V6, 세 tier로 실행한 84회 모두 timeout·형식 오류·4 MiB
초과 없이 성공했다. V6.1의 합계 실행 시간은 `131.981초 → 99.213초`, 최대 RSS는
`187.0 MiB → 135.9 MiB`였다.

공정 Train-only → Dev와 full-public 2,640문항 replay에서 V5 대비 모델 배정
불일치는 모두 `0`이었다. 따라서 점수와 실제 비용도 V5와 정확히 같았다. 먼저
만든 V6.0 Safe는 보수적인 비용 목표로 점수가 `0.009915` 하락했지만, V6.1은
그 변경만 철회하고 운영 hardening을 유지한다.

최종 `linux/arm64` 이미지의 공개 2,640문항 검사는 Fast/Balanced/Premium
`13.006 / 12.671 / 12.725초`로 세 tier 모두 90초 한도를 통과했다. 원본은
[`aggressive-v6-competitive-runtime.json`](../experiments/results/aggressive-v6-competitive-runtime.json)에
있다.
...
| 예산 정책 | V5의 공격적 OOF 검증 정책 | artifact의 V5 정책을 그대로 상속 | 전체 공개 결정 불일치 0 |
...
| V6.1 | `0.695170` | `1.143321` | `1.661573` | `3.072558` | 0 |

V6.1은 세 tier 모두 V5 대비 결정 불일치가 `0/880`이었다. 원본은
[`aggressive-v6-competitive-train-only-dev.json`](../experiments/results/aggressive-v6-competitive-train-only-dev.json)에
있다.
...
| V6.1 | `0.703277` | `1.152316` | `1.744540` | `2.931772` | 0 |

세 tier 모두 V5 대비 결정 불일치가 `0/2,640`이었다. 원본은
[`aggressive-v6-competitive-full-public.json`](../experiments/results/aggressive-v6-competitive-full-public.json)에
있다. V6.0 Safe의 보수적 결과는 기존 `aggressive-v6-*.json` 보고서에 보존한다.
...
| 합계 실행 시간 | `131.981초` | `99.213초` | V6.1 24.8% 감소 |
| 최대 RSS | `187.0 MiB` | `135.9 MiB` | 최대값은 3천만 자 입력 |
| V5 대비 모델 배정 불일치 | - | `0/42 case-tier` | 공격 정책 보존 |
...
기계 판독 원본은
[`edge-cases-v5-v6-competitive.json`](../experiments/results/edge-cases-v5-v6-competitive.json)에
있다.
...
보장할 수 없다. V6.1의 공개·OOF 평가 0회 초과는 검증 근거이지 보장이 아니다.
절대 보장이 최우선이면 유일하게 확실한 정책은 모든
문항을 Light로 보내 비용 비율을 1로 만드는 것이다.

### P1 — 의미적 hard-tail 오분류는 해결되지 않음

`skewed-budget-cliff-5000`에는 쉬운 산술 4,950개와 형식 추론 50개가 있다.
실제 outcome이 없어 정오를 단정할 수는 없지만, V6.1은 V5와 동일하게 쉬운
그룹 일부를 K1에 배정하고 hard-tail에는 K1을 배정하지 않았다. V6.1은 운영 hardening이며
의미 분류 모델을 재학습한 버전이 아니다. 독립 outcome을 확보한 뒤 유형별
worst-group uplift·calibration gate가 필요하다.

### P1 — V5의 공격적 예산 위험도 그대로 유지

공정 Dev와 full-public replay의 결정이 V5와 완전히 같으므로 V5의 점수 상방과
함께 숨은 분포 이동에 대한 비용 위험도 그대로 유지된다. 공개 Train OOF 최악
fold는 `1.200492 / 1.913526 / 3.771347`로 한도 안이지만 절대 보장은 아니다.
...
PYTHONPATH=src python3 tools/benchmark_edge_cases.py \
  ...
  --report-json experiments/results/edge-cases-v5-v6-competitive.json \
  --report-markdown docs/EDGE_CASE_RESULTS_V6_COMPETITIVE.md
...
PYTHONPATH=src python3 tools/evaluate_aggressive_router_v6.py \
  ...
  --report experiments/results/aggressive-v6-competitive-train-only-dev.json
```
